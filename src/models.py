...
class NERSA(torch.nn.Module):
    def __init__(
        ...
        Args:
            ...
            outputs of each LSTM layer except the last layer, with dropout probability
            equal to dropout
        """
        super().__init__()
        ...
        # Create an embedding layer with the given pre-trained weights, use the
        # Embedding.from_pretrained function
        ...
        self.fc_sa: torch.nn.Linear = torch.nn.Linear(
            in_features=2 * hidden_size, out_features=num_SA_labels
        )

    def forward(
        self, inputs: torch.Tensor, text_lengths: torch.Tensor
        ...
        Args:
            ...
            text_lengths (torch.Tensor): Tensor containing the lengths of texts in the
            batch.

        Returns:
            torch.Tensor: The output tensor after passing through the model with the
            logits of NER labels.
        ...
        packed_embedded: PackedSequence = pack_padded_sequence(
        ...
        h0: torch.Tensor = torch.empty(
            (2 * self.num_layers, batch_size, self.hidden_size),
            dtype=torch.float32,
            device=inputs.device,
        )
        c0: torch.Tensor = torch.empty(
            (2 * self.num_layers, batch_size, self.hidden_size),
            dtype=torch.float32,
            device=inputs.device,
        )
